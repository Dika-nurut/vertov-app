...
try {
  ...
  log('reference2video-FAST');
  await page.goto(`${BASE}/generate`, { waitUntil: 'networkidle' });
  await selectModel('seedance-2-0-fast-r2v');
  await setCheap();
  ...
  if (out === 'ok') {
    ...
    log('reference2video-FAST RESULT:', src);
    await page.screenshot({ path: `${SHOT}/vmode-reference2video-FAST.png` });
    console.log('\n✅ reference-to-video STANDARD verified in browser');
  } else {
    const e = await page.textContent('[data-testid="error-state"]').catch(() => '');
    log('reference2video-FAST ERROR:', (e || '').replace(/\s+/g, ' ').slice(0, 90));
    await page.screenshot({ path: `${SHOT}/vmode-reference2video-FAST-ERR.png` });
    console.log('\n❌ blocked');
  }
  await browser.close();
} catch (e) {
